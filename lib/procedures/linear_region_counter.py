...
class LinearRegionCount(object):
    ...
    def calc_LR(self):
        ...
        res = res + res.T # make symmetric, each element in res: A * (1 - B) + (1 - A) * B, a non-zero element indicate a pair of two different linear regions
...
def get_linear_region_counter_v2(train_loader, networks, train_mode=False, num_batch=-1):
    ...
    network_inner_features = [[] for _ in range(len(networks))]
    activations = [[] for _ in range(len(networks))]
    def counting_forward_hook(module_name: str, network_id: int) -> Callable:
        def fn(_, input, output):
            if isinstance(input, tuple) and len(input[0].size()) == 4:
                network_inner_features[network_id].append(output.detach())  # for ReLU
        return fn
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        ...
        for j in range(len(networks)):
            feats = network_inner_features[j]
            if len(feats) == 0:
                continue

            feats = torch.concat([f.view(inputs.size(0), -1) for f in feats], 1)
            activations[j].append(torch.sign(feats).detach())

    activation_scores = []
    for act in activations:
        act = torch.concat(act, 0)
        act = torch.matmul(act.half(), (1 - act).T.half())  # each element in res: A * (1 - B)
        act = act + act.T
        act = 1 - torch.sign(act)
        act = act.sum(1)  # for each sample's linear region: how many identical regions from other samples
        act = 1. / act.float()  # contribution of each redudant (repeated) linear region
        activation_scores.append(act.sum().item())
        torch.cuda.empty_cache()
