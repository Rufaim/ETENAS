...
from procedures import Linear_Region_Collector, get_ntk_n, get_nngp_n
...
def prune_func_rank(xargs, arch_parameters, model_config, model_config_thin, train_loader, valid_loader, lrc_model, search_space, precision=10, prune_number=1):
            ...
            for idx_op in range(len(arch_parameters[idx_ct][idx_edge])):
                ...
                if alpha_active[idx_ct][idx_edge, idx_op] > 0:
                    ...
                    for _ in range(repeat):
                        ...
                        # print(f"Elapsed time: {time.time() - st_time} sec")
                        # exit(1)
                        # ####################
    ...
    for (cell_idx, edge_idx, op_idx), _ in rankings_sum:
...
def prune_func_rank_group(xargs, arch_parameters, model_config, model_config_thin, train_loader, valid_loader, lrc_model, search_space, edge_groups=[(0, 2), (2, 5), (5, 9), (9, 14)], num_per_group=2, precision=10):
    # arch_parameters now has three dim: cell_type, edge, op
    network_origin = get_cell_based_tiny_net(model_config).cuda().train()
    init_model(network_origin, xargs.init)
    network_thin_origin = get_cell_based_tiny_net(model_config_thin).cuda().train()
...
def main(xargs):
    ...
    if xargs.dataset != 'imagenet-1k':
        search_loader, train_loader, valid_loader = get_nas_search_loaders(train_data, valid_data, xargs.dataset, 'configs/dataset_split/', config.batch_size, xargs.workers)
    else:
        ...
        valid_loader = torch.utils.data.DataLoader(valid_data, batch_size=xargs.batch_size, shuffle=True,  num_workers=args.workers, pin_memory=True)
    ...
    while not is_single_path(network):
        ...
        arch_parameters, op_pruned = prune_func_rank(xargs, arch_parameters, model_config, model_config_thin, train_loader, valid_loader, lrc_model, search_space,
    ...
    logger.log(genotypes['arch'][epoch] if type(genotypes['arch'][epoch]) is str else genotypes['arch'][epoch].tostr())

    if xargs.arch_nas_dataset is not None and xargs.search_space_name != 'darts':
        api = API(xargs.arch_nas_dataset)
        logger.log('{:} create API = {:} done'.format(time_string(), api))
        idx = api.query_index_by_arch(genotypes['arch'][epoch])
